...
// Using Bubble Sort to Sort the Medicines
void Sort();
// swap Medicine Name
void swappingName(int i,int j);
// swap a Quantity or Price as resspect to MEDICINE ORDERS(sorting)
void swappingValue(int Quantity[],int i,int j);
// For Admin Block

// To Create a node to add Customers Data
void Create(int num);
// if the Insert Customer the the last node we will utilize this one Function
void InsertEnd();
// to use for insert Customer at the Begining of List
void InsertBegin();
// check the Customer List is Empty or not
bool checkEmpty();
// Delete Customer from Starting of List
void DeleteBegin();
// Delete Customer from End of List
void DeleteEnd();
// used to Delete Customer from required Position
void DeletePosition();
// Display All Data About Customer
void Display();
// this one Function Connected with Create(int n) function for Passing the n as Argument
void TakeOrder();
// This Function is used to Update any required data about Customer
void UpdateOrder();
// this is used to Search any customer to Update thier Record
void Searching();
// used to display All Customers Data
void AllOrdersDetail();
// Check the INPUT id is valid or not
bool CorrectID(int id);
// check the Correct Quantity is Valid or not
bool CorrectQuantity(int id,int quant);
// this one Funtion is Connect with Create function to Add the Medicine for Customers
void AddMedic(node *temp);
// this Function will Display the  individual Customer DATA
void DisplayNodeArray(struct node *temp);
// A funtion used to fetch data from File (customerRecord.txt)
void FetchCustomerData();
// A function used to Save all new or updated data to txt file
void SaveCustomerData();
// show Main menu
void MainMenu();
// show Admin menu
void AdminMenu();
// to show Manager Menu
void ManagerMenu();
// this one function is used to give ID to Updaate Order function
void TakeCustomerPosition();
// show the All Customer Summary for Some Places;
void CustomersSummary();
// this one give ID to Delete Position Function
void RemovePosition();
// this function return time Current
string GetTime();
...
int main()
{
    ...
    choiceLogin=(char)tolower(choiceLogin);
        ...
        case 'e':{
         ...
         case 'g':{
                        cout<<"\t\t\t\tDEAR USER : \n";
                        cout<<"\t\t\t\tAre you Getting Bored : it's Okay\n";
                        cout<<"\t\t\t\t         LETS PLAY GAME           \n";
...
void Welcome(){
      ...
      cout<<"\t\t\t\t\t|           |______|           |\n";
      cout<<"\t\t\t\t\t|______________________________|\n";
      cout<<"\t\t\t\t\t   WELCOME TO DVAGO PHARMACY      \n";
}
...
void swappingName(int i,int j){
    // cout<<i<<" Name to swap : "<<MedicStack[i]<<endl;
    // cout<<j<<" With swap : "<<MedicStack[j]<<endl;
...
void Sort()
{
    for(int i=0;i<recno;i++){
        for(int j=i;j<recno;j++){
            if(MedicStack[i]>MedicStack[j]){
                swappingName(i,j);
            }
        }
    }
...
void SaveData(){
    ...
    ofstream records_output("records.txt"); // create a file stream and open the file for output
        // for(int i=0;i<2;i++){
        //         cout<<i<<MedicStack[i]<<endl;
        // }
    for (int i = 0; i < recno; i++) // write out the new records to screen
	{       
    ...
    choice=(char)tolower(choice);
        switch (choice)
            ...
            {   
                        ...
                        // cout<<"AT THIS RECORD"<<recno<<endl;
...
void FetchCustomerData(){
    ...
    while (getline(records_input, line)) // count all the lines from the file
	{      
        countLine++;// this one count the lines from my file...
            if(countLine==1){
                medic=6;
                quant=7;
                price=8;
                ptr=new node();// cout<<"probelem"<<line<<endl;
                istringstream ( line ) >> num;
                ptr->id=num; // cout<<ptr->id<<endl;
            }
            else if(countLine==2){// cout<<"countLine : "<<countLine<<endl;
                string name=line; // cout<<line<<endl;
...
void Display(){
    node* ptr=head;
    if(ptr!=NULL){
    cout<<"\t\t\t\t\t______CUSTOMER DATA______ \n";
    while (ptr!=NULL)
    {
        cout<<"\t\t\t\t\tID:"<<ptr->id<<") \n";
